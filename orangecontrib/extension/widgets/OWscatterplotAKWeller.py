...
from orangecontrib.extension.utils.widgetAKWeller import OWDataProjectionWidget
...
class OWScatterPlotGraph(OWScatterPlotBase):
    ...
    show_one_line = Setting(False)
    show_zero_line = Setting(False)
    def __init__(self, scatter_widget, parent):
        super().__init__(scatter_widget, parent)
        self.reg_line_items = []
        self.one_line_items = []
        self.zero_line_items = []

    def clear(self):
        super().clear()
        self.reg_line_items.clear()
        self.one_line_items.clear()
        self.zero_line_items.clear()
    ...
    def update_colors(self):
        ...
        self.update_one_line_visibility()
        self.update_zero_line_visibility()
    ...
    def _orthonormal_line(x, y, color, width):
        ...
        r2value = (sxy/(math.sqrt(sxx)*math.sqrt(syy)))**2
        ...
        if sxy != 0:  # also covers sxx != 0 and syy != 0
            ...
            return pg.InfiniteLine(
                QPointF(xmin, start_y),angle=angle,
                pen=pg.mkPen(color=color, width=width),
                label=f"r² = {r2value:.3f}" + ";  y = " + str(np.round(intercept,4)) + "+" +
                      str(np.round(slope,4)) + "x", labelOpts=l_opts)
    ...
    def _add_one_line(self, color, width):

        one_line = pg.InfiniteLine(
            pos=QPointF(0, 0), angle=45,
            pen=pg.mkPen(color=color, width=width))

        self.plot_widget.addItem(one_line)
        self.one_line_items.append(one_line)

    def _add_zero_line(self, color, width):

        zero_line = pg.InfiniteLine(
            pos=QPointF(0, 0), angle=0,
            pen=pg.mkPen(color=color, width=width))

        self.plot_widget.addItem(zero_line)
        self.zero_line_items.append(zero_line)
    ...
    def update_one_line_visibility(self):
        for one_line in self.one_line_items:
            self.plot_widget.removeItem(one_line)
        self.one_line_items.clear()

        if self.show_one_line:
            self._add_one_line(QColor("#505050"), width=4)
        else:
            return

    def update_zero_line_visibility(self):
        for zero_line in self.zero_line_items:
            self.plot_widget.removeItem(zero_line)
        self.zero_line_items.clear()

        if self.show_zero_line:
            self._add_zero_line(QColor("#505050"), width=4)
        else:
            return

class OWScatterPlotAKWeller(OWDataProjectionWidget):
    ...
    description = "Interactive scatter plot visualization with " \
                  "intelligent data visualization enhancements from Orange3 visualization package with extensions by AK Weller"
    ...
    class Inputs(OWDataProjectionWidget.Inputs):
        ...
        #varSpec = Input("Variable specification", Table)
...
if __name__ == "__main__":  # pragma: no cover
    table = Table("iris")
    WidgetPreview(OWScatterPlotAKWeller).run(set_data=table,
