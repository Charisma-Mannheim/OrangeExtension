...
#FILE = 'TestAndScore.log'
#logging.basicConfig(filename=FILE,level=logging.DEBUG, filemode='w', format='%(asctime)s %(levelname)s %(name)s %(message)s')
#logger = logging.getLogger("OWtestandscoreAKWeller")
log = logging.getLogger(__name__)

now = datetime.now()
current_time = now.strftime("%H:%M:%S")
module = "OWtestandscoreAKWeller"
...
class OWTestAndScoreAKWeller(OWWidget):
    name = "Test and score"
    ...
    class Outputs:
        ...
        evaluations_residuals = Output("Residuals", Table)
    ...
    class Error(OWWidget.Error):
        ...
        class_inconsistent = Msg("Test and train data "
    ...
    def _update_controls(self):
        ...
        self.controls.resampling.buttons[
            OWTestAndScoreAKWeller.FeatureFold].setEnabled(enabled)
        self.features_combo.setEnabled(enabled)
        if self.resampling == OWTestAndScoreAKWeller.FeatureFold and not enabled:
            self.resampling = OWTestAndScoreAKWeller.KFold
    ...
    def set_train_data(self, data):
        ...
        if data is not None:
            ...
            if self.fold_feature_selected and bool(self.feature_model):
                self.resampling = OWTestAndScoreAKWeller.FeatureFold
        self._invalidate()
    ...
    def set_test_data(self, data):
        ...
        self.test_data = data
        if self.resampling == OWTestAndScoreAKWeller.TestOnTest:
            self._invalidate()
    ...
    def shuffle_split_changed(self):
        self.resampling = OWTestAndScoreAKWeller.ShuffleSplit
        self._param_changed()

    def _param_changed(self):
        self.modcompbox.setEnabled(self.resampling == OWTestAndScoreAKWeller.KFold)
        self._update_view_enabled()
        self._invalidate()
        self.__update()
    ...
    def update_comparison_table(self):
        ...
        if self.resampling == OWTestAndScoreAKWeller.KFold:
       ...
       # logger.warning('Watch out!')
        ...
        if valid:
            ...
            combined.learner_names = [learner_name(slot.learner)
                                      for slot in valid]
            residual = np.zeros(shape=(combined.predicted.shape))
            for i in range(combined.predicted.shape[0]):
                residual[i,:] = combined.actual - combined.predicted.T[:,i]
            residual = residual.T

            time_series = np.array([[i+1 for i in range(residual.shape[0])]]).T
            actual = np.array([combined.actual]).T
            data1 = np.append(time_series, actual,axis=1)
            data = np.append(data1, residual, axis=1)
            ...
            proposed = ['time series', 'reference data']

            for i in range(len(combined.learner_names)):
                proposed.extend([combined.learner_names[i]])

            meta_name = get_unique_names(proposed, 'object')
            metas = np.array([['object{}'.format(i + 1)
                                  for i in range(residual.shape[0])]],
                                dtype=object).T

            dom = Domain([ContinuousVariable(name, compute_value=lambda _: None) for name in proposed], metas=[StringVariable(name=meta_name)])
            #dom = Domain([name for name in proposed],
             #            metas=[StringVariable(name=meta_name)])
            residuals = Table(dom, data, metas=metas)
        ...
        self.Outputs.evaluations_residuals.send(residuals)
    ...
    def __update(self):
        ...
        if not self.learners:
            ...
            self.commit()
            return
        if self.resampling == OWTestAndScoreAKWeller.KFold and \
        ...
        elif self.resampling == OWTestAndScoreAKWeller.TestOnTest:
        ...
        if self.resampling == OWTestAndScoreAKWeller.TestOnTest:
            ...
            if self.resampling == OWTestAndScoreAKWeller.KFold:
                ...
                sampler = Orange.evaluation.CrossValidation(
                    ...
                    random_state=rstate)
            elif self.resampling == OWTestAndScoreAKWeller.FeatureFold:
                import Orange.evaluation
                sampler = Orange.evaluation.CrossValidationFeature(
                    feature=self.fold_feature)
            elif self.resampling == OWTestAndScoreAKWeller.LeaveOneOut:
                import Orange.evaluation
                sampler = Orange.evaluation.LeaveOneOut()
            elif self.resampling == OWTestAndScoreAKWeller.ShuffleSplit:
                import Orange.evaluation
                sampler = Orange.evaluation.ShuffleSplit(
                    ...
                    random_state=rstate)
            elif self.resampling == OWTestAndScoreAKWeller.TestOnTrain:
                import Orange.evaluation
    ...
    def __submit(self, testfunc):
        ...
        #Hier wird parallelisiert die lineare Regression berechnet
        ...
        self.Outputs.evaluations_residuals.invalidate()
...
if __name__ == "__main__":  # pragma: no cover
    ...
    import orangecontrib.extension.utils.Regression.linearExtension
    prev_learners = [Orange.regression.linear.LinearRegressionLearner(),Orange.regression.linear.RidgeRegressionLearner()]
    #, LDA.Eigenes.Regression.linearExtension.PLSRLearner(),Orange.regression.linear.LassoRegressionLearner(),


    WidgetPreview(OWTestAndScoreAKWeller).run(
